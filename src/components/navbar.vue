<template>
  <header class="c-navbar">
    <div
      class="c-disclaimer"
      v-if="!closedDisclaimer && disclaimerLink.hasOwnProperty('name')"
    >
      <a
      ...
      <a class="c-disclaimer__close" href="#" @click="closeDisclaimer"
        ><tipi-icon icon="close" color="#fff"
      /></a>
    </div>
    <div
      v-if="preImage"
      class="c-decorator"
      :style="`background-image: url(${preImage})`"
    ></div>
    <div class="c-navbar__wrapper o-container">
      ...
      <p class="c-navbar__subtitle">
        Escáner de ODS de la Cooperación Española
      </p>
      <button
        type="button"
      ...
      <nav class="c-navbar__menu" :class="{ 'is-active': menuVisible }">
        <ul class="c-menu">
          ...
          >
            <a v-if="link.external" :href="link.route" target="_blank"
              >{{ link.name
              }}<tipi-icon
                :icon="link.icon"
                class="c-menu__icon"
